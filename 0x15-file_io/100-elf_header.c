...
/**
 * return_error_msg - return error message
 * @code: error code
 * @msg: error message
 * @...: rest arguments
*/

void return_error_msg(int code, char * msg, ...)
{
    va_list args;
...
int main(int ag, char **av)
{
    ...
    if (ag != 2)
        return_error_msg(98, "Usage: elf_header elf_filename");

    file_desc = open(av[1], O_RDONLY);
    if (file_desc == -1)
        return_error_msg(98, "Error: Can't open file %s\n", av[1]);
    
    if ((read(file_desc, &header, sizeof(header))) != sizeof(header))
        return_error_msg(98, "Error: Cannot read ELF header\n");
    ...
    if (header.e_ident[EI_MAG0] != ELFMAG0 ||
        ...
        {
            return_error_msg(98, "Error: %s file is not an ELF file", av[1]);
        }
